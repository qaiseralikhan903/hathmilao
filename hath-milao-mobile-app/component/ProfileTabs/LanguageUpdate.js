...
export default class SkillsUpdate extends Component {
  ...
  componentWillMount() {
    const  profile  = this.props.navigation.getParam("data", "");

    this.setState({
      skill: profile.skill ? profile.skill : [],
      language: profile.language,
      cvurl: "Http://HElloWOrld.com/KalBilalKarayga",
      looking: profile.looking ? profile.looking : false,
      desiredjob: "desired",
      array: profile.language ? profile.language : []
    });
  }
  ...
  async onUpdate() {
    ...
    // console.log(data);
    await this.setState({ visible: true, language: data });
