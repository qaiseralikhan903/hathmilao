...
import { FYPApplicant } from "../../services/fyp.service";

import StudentProfile from "../Helper/StudentProfile.jsx";
import { Spin } from "antd";

export default class ApplicantsFYP extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fypid: null,
  ...
  componentDidMount() {
    this.setState({
      fypid: this.props.fypid,
      Loading: false
    });

    FYPApplicant(1, this.props.fypid)
      .then(response => {
        const pages = [];
  ...
  handlePageChange = index => {
    ...
    FYPApplicant(index, this.state.fypid)
      .then(response => {
        this.setState({
          docs: response.data.students.docs,
          currentPage: response.data.students.page,
      ...
      <>
        {this.state.Loading ? (
          ...
          <CardBody>
            ...
            >
              <Spin
                tip="Loading Final Year Project Applicants..."
                size="large"
              />
            </div>
          </CardBody>
